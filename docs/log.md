...
* **Update**: Streamlined the repository and `docs/` by removing off-topic prototype files (`agy_sample.py`, hybrid workflow and local LLM setup notes) to focus exclusively on `logit-gate-rag`'s core architecture, Ruri embedding, and hardware benchmark evaluations.
